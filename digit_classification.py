...
def compute_activations(x, W, B):
    ...
    for i in range(0, len(W)):
    	# calculating z and a for layer i
        z=np.dot(W[i],a) -  B[i]
...
# Using t, the true label for the data point and z and a values,
...
def backpropagation(x, t, W, B):
    ...
    delta=d_b[:]
    delta[-1]=derivative_activation(z_s[-1])*(t-a_s[-1])

    l=len(W)
    for i in range(l-1,0,-1):
        delta[i-1]=np.dot(np.transpose(W[i]), delta[i])*derivative_activation(z_s[i])

    for i in range(l-1,-1,-1):
       d_w[i]=np.dot(delta[i], np.transpose(a_s[i]))
       d_b[i]=-1*delta[i]
